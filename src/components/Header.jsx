...
export default function Header() {
  ...
  const navLinks = roleConfig.navigation;

  // Extract color classes based on role
  ...
  return (
    <header className={`sticky top-0 z-50 border-b ${headerStyle.borderColor} bg-background/80 backdrop-blur-md`}>
      <div className="container flex h-16 items-center justify-between">
        <Link to="/" className="flex items-center gap-2">
          <span className={`font-display text-xl font-bold ${headerStyle.accentColor}`}>AgriConnect</span>
        </Link>

        {/* Desktop nav */}
        <nav className="hidden items-center gap-6 md:flex">
          {navLinks.map((l) =>
          <Link key={l.to} to={l.to} className={`font-body text-sm font-medium text-muted-foreground transition-colors ${headerStyle.hoverColor}`}>
              {l.label}
            </Link>
          )}
        </nav>
      ...
      <div className="border-t bg-background p-4 md:hidden animate-fade-in">
          <nav className="flex flex-col gap-3">
            {navLinks.map((l) =>
          <Link key={l.to} to={l.to} onClick={() => setMenuOpen(false)} className={`font-body text-sm font-medium text-muted-foreground ${headerStyle.hoverColor}`}>
                {l.label}
              </Link>
          )}
            <div className="mt-3 flex flex-col gap-2">
              {user ?
            <>
                  <Link to="/profile" onClick={() => setMenuOpen(false)}><Button variant="outline" className="w-full">Profile</Button></Link>
